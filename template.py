...
app.config['SQLALCHEMY_DATABASE_URI'] = environ.get(
    'dbURL') or 'mysql://MrLeKB:Ydauu2w7@spmdatabase.cffxztdrjjzg.ap-southeast-1.rds.amazonaws.com/lnd_database'
...
class Question(db.Model):
    ...
    QuizID = db.Column(db.String(7), db.ForeignKey(
        Quiz.QuizID), primary_key=True)
...
class Option(db.Model):
    ...
    QuizID = db.Column(db.String(7), db.ForeignKey(
        Quiz.QuizID), primary_key=True)
    QuestionID = db.Column(db.String(7), db.ForeignKey(
        Question.QuestionID), primary_key=True)
...
class Section(db.Model):
    ...
    SectionID = db.Column(db.String(2), primary_key=True)
    CourseID = db.Column(db.String(5), ForeignKey(
        Course.CourseID), primary_key=True)
    ClassID = db.Column(db.String(2), ForeignKey(
...
def create_user():
    ...
    user = User(UserID=request_data['UserID'],
                UserRole=request_data['UserRole'], EmployeeID=request_data['EmployeeID'])
    try:
        db.session.add(user)
        db.session.commit()
...
def coursematerial_by_id(MaterialID):
    coursematerial = CourseMaterial.query.filter_by(
...
def get_sectionmaterial(CourseID, ClassID, SectionID):
    ...
    sectionmaterial = SectionMaterial.query.filter_by(
        CourseID=CourseID, ClassID=ClassID, SectionID=SectionID).first()
    if sectionmaterial:
        return jsonify({
...
def get_all_sectionmaterial(CourseID, ClassID):
    ...
    sectionmaterial_list = SectionMaterial.query.filter_by(
        CourseID=CourseID, ClassID=ClassID).all()
...
def question_by_id(questionID, quizID):
    question = Question.query.filter_by(
        QuestionID=questionID, QuizID=quizID).first()
    if question:
        return jsonify({
...
def add_quiz():
    ...
    section = Section.query.filter_by(
        CourseID=data["CourseID"], ClassID=data["ClassID"], SectionID=data["SectionID"]).first()
...
def get_class(CourseID, ClassID):
    Class_Entry = Class.query.filter_by(
        CourseID=CourseID, ClassID=ClassID).first()
    if Class_Entry:
        return jsonify({
...
def create_enrolRecord_Assign():
    try:
        ...
        try:
            Class_Entry = Class.query.filter_by(
                CourseID=data['CourseID'], ClassID=data['ClassID']).first()
        ...
        Enrollist = EnrolRecord.query.filter_by(
            CourseID=data['CourseID'], ClassID=data['ClassID'], Acceptance="confirmed").all()

        CoursePreRequistes = PreRequisite.query.filter_by(
            CourseID=data['CourseID']).all()
        StudentCompletedCourses = CourseHistory.query.filter_by(
            LearnerID=data['LearnerID'], Completed="T").all()
        ...
        try:
            learnerEnrolled = EnrolRecord.query.filter_by(
                CourseID=data['CourseID'], LearnerID=data['LearnerID']).first()
            if learnerEnrolled:
                return jsonify({"code": 500,
...
def accept_learner():
    try:
        ...
        if len(Enrollist) < class_size:
            learner = EnrolRecord.query.filter_by(
                CourseID=data['CourseID'], ClassID=data['ClassID'], LearnerID=data['LearnerID']).first()
...
def add_learner():
    try:
        ...
        CoursePreRequistes = PreRequisite.query.filter_by(
            CourseID=data['CourseID']).all()
        StudentCompletedCourses = CourseHistory.query.filter_by(
            CourseID=data["CourseID"], LearnerID=data["LearnerID"], ClassID=data["ClassID"], Completed="T").all()
        ...
        try:
            learnerEnrolled = CourseHistory.query.filter_by(
                CourseID=data['CourseID'], LearnerID=data['LearnerID']).first()
            if learnerEnrolled:
                return jsonify({"code": 500,
        ...
        try:
            learner_S_Enrolled = EnrolRecord.query.filter_by(
                CourseID=data['CourseID'], ClassID=data['ClassID'], LearnerID=data['LearnerID']).first()
...
def remove_learner():
    try:
        ...
        print(data)
        try:
            Enrol_Record = EnrolRecord.query.filter_by(
                CourseID=data['CourseID'], ClassID=data["ClassID"], LearnerID=data['LearnerID']).first()
        except Exception as e:
            return jsonify({"code": 500,
        ...
        Class_Record = Class.query.filter_by(
            CourseID=data['CourseID'], ClassID=data["ClassID"]).first()
...
if __name__ == '__main__':
    print("This is flask for " +
          os.path.basename(__file__) + ": student records ...")
